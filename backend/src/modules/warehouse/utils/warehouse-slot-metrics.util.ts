...
export function calculateSlotMetrics(
  ...
  const capacityM3 = Number(capacityM3Input);

  const calculatedSlotUsedM3 = Number(
    activeStoredGoods.reduce((sum, g) => sum + Number(g.volumeM3), 0).toFixed(2),
  );
  const actualSlotUsedM3 =
    calculatedSlotUsedM3 > 0 ? calculatedSlotUsedM3 : Number(initialSlotUsedM3);
  ...
  const actualSlotUsedWeightKg = Number(
    activeStoredGoods.reduce((sum, g) => sum + Number(g.weightKg || 0), 0).toFixed(2),
  );
