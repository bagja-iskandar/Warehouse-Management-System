...
export default function AdminDashboardPage() {
  ...
  return (
    <DashboardContainer>
      ...
      {/* 2. Standardized 4 KPI Metric Summary Cards (Compact) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
          ...
          subtext={
            <span className="flex items-center justify-between text-[10px] font-mono">
              ...
              <span className="font-semibold text-slate-700">
                {coldZone.occupiedSlots} / {coldZone.totalSlots} ({coldZone.occupancyPercent.toFixed(0)}%)
              </span>
            </span>
          }
        />
        ...
        <DashboardMetricCard
          ...
          subtext={
            <span className="text-[10px] text-slate-500">
              {logistics.reeferVehiclesCount} reefer & {Math.max(0, logistics.totalVehicles - logistics.reeferVehiclesCount)} dry trucks active
            </span>
          }
        />
        ...
        <DashboardMetricCard
          ...
          theme="emerald"
          badge={
            <span className="text-[10px] text-emerald-800 bg-emerald-100 font-semibold px-2 py-0.5 rounded-md">
      ...
      <div className="bg-white border border-slate-200/90 rounded-2xl p-3 shadow-xs space-y-2">
        <div className="flex items-center justify-between pb-1.5 border-b border-slate-100 px-0.5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-slate-800">Admin Operational Shortcuts</span>
            <span className="text-[10.5px] text-slate-400 hidden sm:inline">• Direct pathways for inventory, rack capacity, logistics & billing</span>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          ...
          >
            <div className="h-7 w-7 rounded-lg bg-sky-50 text-sky-600 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-stretch">
        {/* Left Column (7 cols): Zone Capacity Breakdown + Active Dispatch Queue */}
        <div className="lg:col-span-7 flex flex-col gap-5 h-full">
            ...
            <div className="space-y-3">
              ...
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2">
                    <div className="h-5.5 w-5.5 rounded-md bg-sky-100 text-sky-700 flex items-center justify-center font-bold">
                      <Snowflake className="h-3 w-3" />
                    </div>
                    <div>
                      <span className="font-bold text-slate-900">Cold Storage Facility</span>
                      <span className="text-slate-400 text-[10.5px] ml-1.5 font-mono">(-18.0°C to -22.0°C)</span>
                    </div>
                  </div>
                  <div className="text-right font-mono">
                    <span className="font-bold text-slate-800">{coldZone.usedM3} m³</span>
                    <span className="text-slate-400 text-[10.5px]"> / {coldZone.capacityM3} m³ ({coldZone.occupancyPercent.toFixed(1)}%)</span>
                  </div>
                </div>
              ...
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2">
                    <div className="h-5.5 w-5.5 rounded-md bg-emerald-100 text-emerald-700 flex items-center justify-center font-bold">
                      <Warehouse className="h-3 w-3" />
                    </div>
                    <div>
                      <span className="font-bold text-slate-900">Standard Dry Storage</span>
                      <span className="text-slate-400 text-[10.5px] ml-1.5 font-mono">(Ambient 20°C - 25°C)</span>
                    </div>
                  </div>
                  <div className="text-right font-mono">
                    <span className="font-bold text-slate-800">{standardZone.usedM3} m³</span>
                    <span className="text-slate-400 text-[10.5px]"> / {standardZone.capacityM3} m³ ({standardZone.occupancyPercent.toFixed(1)}%)</span>
                  </div>
                </div>
                <div className="w-full bg-slate-200 rounded-full h-1.5 overflow-hidden">
                  <div
              ...
              <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2">
                    <div className="h-5.5 w-5.5 rounded-md bg-indigo-100 text-indigo-700 flex items-center justify-center font-bold">
                      <Boxes className="h-3 w-3" />
                    </div>
                    <div>
                      <span className="font-bold text-slate-900">Heavy Duty & Staging Area</span>
                      <span className="text-slate-400 text-[10.5px] ml-1.5 font-mono">(High-Bay Pallet Racking)</span>
                    </div>
                  </div>
                  <div className="text-right font-mono">
                    <span className="font-bold text-slate-800">{heavyDutyZone.usedM3} m³</span>
                    <span className="text-slate-400 text-[10.5px]"> / {heavyDutyZone.capacityM3} m³ ({heavyDutyZone.occupancyPercent.toFixed(1)}%)</span>
                  </div>
                </div>
                <div className="w-full bg-slate-200 rounded-full h-1.5 overflow-hidden">
                  <div
          ...
          {/* Active Logistics & Dispatch Queue (Flex-1 to stretch & match bottom line) */}
          <DashboardSectionCard
            compact={true}
            ...
            {activeOrders.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center py-6 px-4 text-center bg-slate-50/60 border border-dashed border-slate-200 rounded-xl my-auto space-y-2.5">
                ...
                <div className="space-y-0.5 max-w-sm">
                  <h3 className="text-xs font-bold text-slate-700">No Active Logistics Orders</h3>
                  <p className="text-[11px] text-slate-400 leading-relaxed">
                    There are currently no active delivery orders in progress. Dispatch queues will appear here automatically.
                  </p>
                </div>
                <Link href="/admin/logistics" className="pt-1">
              ...
              <div className="overflow-x-auto my-auto">
                <table className="w-full text-left text-xs">
                  <thead className="bg-slate-50 text-slate-400 uppercase text-[9.5px] font-bold">
                    <tr>
                      <th className="py-2 px-2.5">Order Number</th>
                      <th className="py-2 px-2.5">Destination</th>
                      <th className="py-2 px-2.5">Fleet Unit</th>
                    ...
                    {activeOrders.map((order) => (
                      <tr key={order.id} className="hover:bg-slate-50/80 transition-colors">
                        <td className="py-2 px-2.5">
                          ...
                          <span className="text-[10px] text-slate-500">{order.customerName || "Verified Customer"}</span>
                        </td>
                        <td className="py-2 px-2.5 max-w-[180px]">
                          ...
                          <span className="text-[9.5px] text-slate-400">Created: {new Date(order.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                        </td>
                        <td className="py-2 px-2.5">
                          <span className="font-mono text-slate-700 font-semibold block">{order.vehiclePlate || "Fleet Assigned"}</span>
                          <span className="text-[9.5px] text-slate-400">{order.requiresReefer ? "❄️ Reefer Cold" : "📦 Dry Freight"}</span>
                        </td>
                        <td className="py-2 px-2.5">
                          <Badge
        ...
        {/* Right Column (5 cols): Live Cold Telematics + Loading Dock Status */}
        <div className="lg:col-span-5 flex flex-col gap-5 h-full">
          ...
          >
            <div className="space-y-2.5">
              <div className="flex items-center justify-between p-2.5 rounded-xl bg-sky-50/80 border border-sky-100">
                <div>
                  <span className="text-[10.5px] text-sky-800 font-semibold block">Freezer Chamber Alpha</span>
                  <span className="text-base font-extrabold text-sky-950 font-mono">-18.4°C</span>
                </div>
                <Badge className="bg-sky-600 text-white text-[9.5px]">Optimal</Badge>
              </div>

              <div className="flex items-center justify-between p-2.5 rounded-xl bg-sky-50/80 border border-sky-100">
                <div>
                  <span className="text-[10.5px] text-sky-800 font-semibold block">Freezer Chamber Beta</span>
                  <span className="text-base font-extrabold text-sky-950 font-mono">-19.1°C</span>
                </div>
                <Badge className="bg-sky-600 text-white text-[9.5px]">Optimal</Badge>
              </div>
              ...
              <div className="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 border border-slate-200">
                <div>
                  <span className="text-[10.5px] text-slate-600 font-semibold block">Chiller Inbound Dock</span>
                  <span className="text-sm font-bold text-slate-900 font-mono">+4.2°C</span>
                </div>
                <Badge variant="outline" className="text-[9.5px]">Active</Badge>
              </div>
          ...
          {/* Loading Dock & Staging Bay Status (Flex-1 to stretch & match bottom line) */}
          <DashboardSectionCard
            compact={true}
            className="flex-1 flex flex-col justify-between"
            bodyClassName="flex-1 flex flex-col justify-between p-3.5 sm:p-4"
            ...
            <div className="space-y-2.5 flex-1 flex flex-col justify-between">
              <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <span className="text-[10.5px] text-slate-500 block font-mono">B 9876 XYZ • Loading Outbound</span>
                </div>
                <Badge className="bg-indigo-50 text-indigo-700 text-[9.5px]">Active</Badge>
              </div>
              ...
              <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <span className="text-[10.5px] text-slate-500 block font-mono">D 1234 ABC • Unloading Cargo</span>
                </div>
                <Badge className="bg-amber-50 text-amber-700 text-[9.5px]">Occupied</Badge>
              </div>
              ...
              <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <span className="text-[10.5px] text-slate-400 block font-mono">Vacant • Standby for Dispatch</span>
                </div>
                <Badge variant="outline" className="text-[9.5px] text-slate-500">Available</Badge>
              </div>
            </div>
          </DashboardSectionCard>
        </div>
            ...
            {recentActivities.length === 0 ? (
              ...
              <div className="space-y-2.5 max-h-[220px] overflow-y-auto pr-1">
                {recentActivities.slice(0, 4).map((act: any, idx: number) => (
                  <div key={idx} className="flex items-start gap-2.5 p-2 rounded-xl hover:bg-slate-50 transition-colors">
