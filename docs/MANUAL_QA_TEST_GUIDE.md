...
### TC-016
**Module:** Physical Rack Matrix & Zone Allocation  
**Role:** Warehouse Admin (`admin@wms.id`)  
**Scenario:** Inspeksi Mendalam Rack Slot Multi-Tenant & Penelusuran Barang Tersimpan  
**Priority:** Critical  

**Precondition:**
- Login sebagai Warehouse Admin (`admin@wms.id`).
- Buka menu **"Capacity & Rack Grid"** pada [http://localhost:3000/admin/warehouse/capacity](http://localhost:3000/admin/warehouse/capacity).

**Steps:**
1. Pada visual grid rack, cari slot berstatus occupied/partial (misal `COLD-B01` atau `COLD-B02`).
2. Periksa tampilan ringkasan card sebelum dibuka:
   - Pastikan menampilkan format multi-tenant yang akurat (misal `1 Customer • 10 Packages (1 SKU)` atau `2 Customers • 30 Packages (2 SKUs)`).
   - Pastikan menampilkan **Dual Progress Bar**: Bar Volume ($m^3$) dan Bar Beban Berat ($kg$).
   - Pastikan menampilkan telemetri suhu real-time (misal `-19.2°C`).
3. Klik card rack `COLD-B01` untuk membuka **`SlotDetailModal`**.
4. Di dalam modal detail:
   - Periksa 4 kartu KPI di bagian atas: *Volume Capacity*, *Weight Load Capacity*, *Tenant & Packages*, dan *Sensor Telemetry*.
   - Periksa tabel inventaris multi-tenant: Customer (Nama & Perusahaan), Nama Barang, SKU Barcode, Quantity, Dimensi ($L \times W \times H$), Volume total ($m^3$), Berat total ($kg$), Status, dan Suhu.
   - Coba gunakan search bar di atas tabel: Ketik nama tenant atau SKU barcode untuk menguji filter instan.
   - Klik ikon copy 📋 pada SKU barcode untuk memverifikasi toast notifikasi salin barcode.
   - Klik tombol **"Print Slot QR"** untuk melihat label barcode QR siap cetak.

**Expected Result:**
- Modal terbuka dengan animasi halus (*smooth zoom-in*), menampilkan data aktual dari PostgreSQL.
- Tabel menampilkan seluruh entitas barang yang secara fisik berstatus `STORED` di dalam slot tersebut.
- Barang berstatus `WAITING_FOR_INBOUND`, `DRAFT`, atau `CANCELLED` **SAMA SEKALI TIDAK TAMPIL** di dalam slot fisik.
- Fitur pencarian memfilter baris tabel secara instan tanpa glitch.

**What I should observe manually:**
- Indikator kapasitas bottleneck (misal *Normal Load*, *Near Volume Capacity*, *Near Weight Capacity*, atau *Fully Saturated*) menampilkan badge warna yang sesuai.
...
### TC-017
**Module:** Physical Rack Matrix — Dual Capacity Validation  
**Role:** Warehouse Admin (`admin@wms.id`)  
**Scenario:** Uji Ketat Penolakan Kapasitas Independen (Volume $m^3$ vs Beban Berat $kg$)  
**Priority:** Critical  

**Precondition:**
- Terdapat slot rak dengan kapasitas volume $100.00\text{ m}^3$ dan batas beban struktural $5.000.0\text{ kg}$ (e.g. `COLD-B01`).
- Slot telah terisi barang ringan namun bervolume besar ($90.0\text{ m}^3$, $1.000\text{ kg}$). Sisa volume: $10.0\text{ m}^3$, Sisa beban: $4.000\text{ kg}$.

**Steps:**
1. **Uji Kasus 1 (Pelanggaran Volume):**
   - Coba lakukan Put-Away atau pendaftaran barang baru dengan volume $15.0\text{ m}^3$ dan berat $500\text{ kg}$ ke slot `COLD-B01`.
2. **Uji Kasus 2 (Pelanggaran Beban Berat):**
   - Coba lakukan Put-Away barang dengan volume kecil $5.0\text{ m}^3$ tetapi berat $4.500\text{ kg}$ ke slot `COLD-B01`.

**Expected Result:**
- **Uji Kasus 1:** Sistem menolak alokasi dengan pesan error deskriptif dalam Bahasa Inggris:
  `"Cannot put away this inventory. Rack volume capacity would be exceeded by 5.00 m³. Slot 'COLD-B01' only has 10.00 m³ available, while this goods requires 15.00 m³."`
- **Uji Kasus 2:** Sistem menolak alokasi meskipun volume cukup, karena batas beban struktural terlampaui:
  `"Cannot put away this inventory. Rack weight capacity would be exceeded by 500.0 kg. Slot 'COLD-B01' only has 4000.0 kg load capacity available, while this goods weighs 4500.0 kg."`
- Database PostgreSQL menjaga integritas data tanpa terjadi alokasi ganda atau overflow (*zero corruption*).

**What I should observe manually:**
- Pesan error tampil jelas pada toast merah atau banner form tanpa merusak input yang telah diisi.
...
### TC-018
**Module:** Physical Rack Matrix — Intra-Rack Relocation  
**Role:** Warehouse Admin (`admin@wms.id`)  
**Scenario:** Pemindahan / Relokasi Barang Antar-Slot Rak dengan Rekalkulasi Kapasitas Otomatis  
**Priority:** High  

**Precondition:**
- Slot `COLD-B01` berisi barang `Norwegian Salmon Fillet Grade A` ($3.00\text{ m}^3$, $500\text{ kg}$).
- Slot tujuan `COLD-B02` dalam kondisi vacant / tersedia.
- Buka modal detail `SlotDetailModal` untuk `COLD-B01`.

**Steps:**
1. Klik tombol **"Move"** (atau **"Relocate Cargo"**) pada baris barang *Salmon Fillet*.
2. Pada form relokasi intra-rack:
   - Target Destination Slot: Pilih `COLD-B02`. Perhatikan info kapasitas yang tertera pada dropdown (`Avail: 100.0 m³ / 5000 kg [VACANT]`).
   - Transfer Reason: Masukkan `Optimasi distribusi beban cold chain rack`.
   - Notes: Masukkan `Dipindahkan ke rak tier bawah untuk mempermudah akses forklift`.
3. Klik tombol **"Confirm Relocation"**.

**Expected Result:**
- Request `PATCH /api/v1/goods/:id/transfer-slot` berhasil diproses dalam transaksi atomic database.
- Kapasitas slot asal `COLD-B01` berkurang $-3.00\text{ m}^3$ dan $-500\text{ kg}$.
- Kapasitas slot tujuan `COLD-B02` bertambah $+3.00\text{ m}^3$ dan $+500\text{ kg}$.
- Visual grid rack di halaman `/admin/warehouse/capacity` langsung ter-update secara reaktif.

**What I should observe manually:**
- Animasi tombol menampilkan spinner loading selama relokasi berlangsung dan toast sukses hijau muncul setelah selesai.
...
### ⚖️ Rumus 2: Perhitungan Dual Kapasitas Slot Rak (Volume & Beban Berat)
$$\text{Batas Maksimal Beban (kg)} = \text{Kapasitas Volume Slot (m}^3) \times 50\text{ kg/m}^3$$
$$\text{Sisa Volume Tersedia (m}^3) = \max(0, \text{Kapasitas Volume} - \sum \text{Volume Barang STORED})$$
$$\text{Sisa Beban Tersedia (kg)} = \max(0, \text{Batas Maksimal Beban} - \sum \text{Berat Barang STORED})$$

*Syarat Mutlak Alokasi Put-Away / Relokasi:*
$$\text{Volume Barang Masuk} \le \text{Sisa Volume Tersedia} \quad \mathbf{DAN} \quad \text{Berat Barang Masuk} \le \text{Sisa Beban Tersedia}$$

*Contoh Validasi:*
- Slot Rak: Kapasitas $100\text{ m}^3 \rightarrow \text{Beban Maksimal: } 100 \times 50 = 5.000\text{ kg}$.
- Terisi: Barang A ($20\text{ m}^3$, $3.500\text{ kg}$).
- Sisa Volume: $100 - 20 = \mathbf{80\text{ m}^3}$.
- Sisa Beban: $5.000 - 3.500 = \mathbf{1.500\text{ kg}}$.
- Jika masuk Barang B ($10\text{ m}^3$, $2.000\text{ kg}$) $\rightarrow$ **DITOLAK** karena $2.000\text{ kg} > 1.500\text{ kg}$ meskipun volume cukup ($10 < 80$).

---

### 💰 Rumus 3: Perhitungan Biaya Sewa Gudang Bulanan (IDR)
...
### ⚠️ Rumus 4: Denda Keterlambatan Pembayaran (Penalty Fee)
...
| **6. Physical Rack Matrix & Dual Capacity**| 3 | [ ] | [ ] | [ ] | *Pending Manual Run* |
| **7. Outbound Delivery & POD** | 2 | [ ] | [ ] | [ ] | *Pending Manual Run* |
| **8. Billing, Invoices & Payments**| 2 | [ ] | [ ] | [ ] | *Pending Manual Run* |
| **9. Multi-Tenant Privacy** | 1 | [ ] | [ ] | [ ] | *Pending Manual Run* |
| **10. System Notifications** | 1 | [ ] | [ ] | [ ] | *Pending Manual Run* |
| **11. UI/UX & Exploratory** | 2 | [ ] | [ ] | [ ] | *Pending Manual Run* |
| **TOTAL SCORE** | **25 Skenario** | **0** | **0** | **0** | **READY FOR UAT** |
